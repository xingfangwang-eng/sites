...
function ComparisonTable({ comparisonData, brandColor }: { comparisonData?: any[], brandColor: string }) {
  ...
  return (
    <div style={{ backgroundColor: '#f3e8ff', padding: '24px', marginBottom: '24px' }}>
      <h2 style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '16px', color: '#000000' }}>How We Compare</h2>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px', color: '#000000' }}>
          <thead>
            <tr style={{ backgroundColor: '#e9d5ff' }}>
              <th style={{ padding: '12px', textAlign: 'left', borderBottom: `2px solid ${brandColor}`, color: '#000000' }}>Feature</th>
              <th style={{ padding: '12px', textAlign: 'center', borderBottom: `2px solid ${brandColor}`, color: '#000000' }}>Us</th>
              <th style={{ padding: '12px', textAlign: 'center', borderBottom: `2px solid ${brandColor}`, color: '#000000' }}>Notion</th>
              <th style={{ padding: '12px', textAlign: 'center', borderBottom: `2px solid ${brandColor}`, color: '#000000' }}>Obsidian</th>
              <th style={{ padding: '12px', textAlign: 'center', borderBottom: `2px solid ${brandColor}`, color: '#000000' }}>Evernote</th>
            </tr>
          </thead>
          <tbody>
            {data.map((row, index) => (
              <tr key={index} style={{ backgroundColor: index % 2 === 0 ? '#faf5ff' : '#ffffff' }}>
                <td style={{ padding: '12px', borderBottom: '1px solid #e5e7eb', color: '#000000' }}>{row.feature}</td>
                <td style={{ padding: '12px', textAlign: 'center', borderBottom: '1px solid #e5e7eb', fontWeight: 'bold', color: '#22c55e' }}>{row.noaimd}</td>
                <td style={{ padding: '12px', textAlign: 'center', borderBottom: '1px solid #e5e7eb', color: '#000000' }}>{row.notion}</td>
                <td style={{ padding: '12px', textAlign: 'center', borderBottom: '1px solid #e5e7eb', color: '#000000' }}>{row.obsidian}</td>
                <td style={{ padding: '12px', textAlign: 'center', borderBottom: '1px solid #e5e7eb', color: '#000000' }}>{row.evernote}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function PitfallGuide({ pitfalls }: { pitfalls?: { title: string; description: string }[] }) {
  ...
  return (
    <div style={{ backgroundColor: '#fffbeb', padding: '24px', marginBottom: '24px', border: '2px solid #f59e0b', borderRadius: '8px' }}>
      <h2 style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '16px', color: '#000000' }}>⚠️ Pitfall Guide</h2>
      <div style={{ fontSize: '14px', color: '#000000', lineHeight: '1.6' }}>
        ...
        {data.map((pitfall, index) => (
          <div key={index} style={{ marginBottom: index < data.length - 1 ? '12px' : '0', padding: '12px', backgroundColor: '#fef3c7', borderRadius: '4px' }}>
            <strong style={{ color: '#000000' }}>Pitfall {index + 1}: {pitfall.title}</strong><br/>
            <span style={{ color: '#000000' }}>{pitfall.description}</span>
          </div>
        ))}
      </div>
    </div>
...
function RelatedPages({ pages }: { pages: SEOData[] }) {
  return (
    <div style={{ backgroundColor: '#fdf4ff', padding: '24px', marginBottom: '24px', border: '2px solid #d946ef' }}>
      <h2 style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '16px', color: '#000000' }}>You May Also Like</h2>
...
export default function BaseSEOLayout({
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#ffffff', fontFamily: 'sans-serif', color: '#000000' }}>
      {/* 导航栏 */}
      <nav style={{ 
        backgroundColor: brandColor, 
        padding: '12px 20px', 
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px 16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', gap: '32px', flexWrap: 'wrap' }}>
          ...
          <div style={{ width: '100%', flex: '2' }}>
            <h1 style={{ fontSize: '32px', fontWeight: 'bold', marginBottom: '32px', color: '#000000' }}>{data.title}</h1>
            ...
            {/* 解决方案 */}
            <div style={{ backgroundColor: '#e6f7ff', padding: '24px', marginBottom: '24px' }}>
              <h2 style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '16px', color: '#000000' }}>The Solution</h2>
              <p style={{ color: '#000000' }}>{data.how_to_solve}</p>
            </div>
            ...
            {siteConfig.benefits ? (
              siteConfig.benefits.map((benefit, index) => (
                <div key={index} style={{ backgroundColor: '#f6ffed', padding: '24px', marginBottom: '24px' }}>
                  <h2 style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '16px', color: '#000000' }}>{benefit.title}</h2>
                  <p style={{ color: '#000000' }}>{benefit.description}</p>
                </div>
              ))
            ) : (
              <>
                <div style={{ backgroundColor: '#f6ffed', padding: '24px', marginBottom: '24px' }}>
                  <h2 style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '16px', color: '#000000' }}>Zero Latency</h2>
                  <p style={{ color: '#000000' }}>Opens in under 100ms. No AI bloat slowing you down.</p>
                </div>

                <div style={{ backgroundColor: '#f6ffed', padding: '24px', marginBottom: '24px' }}>
                  <h2 style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '16px', color: '#000000' }}>100% Private</h2>
                  <p style={{ color: '#000000' }}>Your data never leaves your device. No cloud, no tracking.</p>
                </div>
              </>
            )}
        ...
        <div style={{ marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #e5e7eb', textAlign: 'center' }}>
          <p style={{ marginBottom: '8px', color: '#000000' }}>
            <strong style={{ color: '#000000' }}>{siteName}</strong> - <span style={{ color: '#000000' }}>{footerText}</span>
          </p>
        </div>
      </div>
    </div>
